...
void CheckTotal_8_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void CheckTotal_8_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 32;}
...
mySubcomponents[aux_create+i] = aux_cmp_num;
MultiMux1_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 4 ;
aux_cmp_num += 1;
}
}
...
ctx->componentMemory[coffset].templateName = "CheckTotal";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "ltPlayer";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&signalValues[mySignalStart + 1]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "gtResult";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mux1";
mySubcomponents[aux_create+i] = aux_cmp_num;
Mux1_6_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 8 ;
aux_cmp_num += 2;
}
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mux1Equal";
mySubcomponents[aux_create+i] = aux_cmp_num;
Mux1_6_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 8 ;
aux_cmp_num += 2;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "sumPlayerCheck";
mySubcomponents[aux_create+i] = aux_cmp_num;
CheckTotal_8_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 18 ;
aux_cmp_num += 4;
}
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "sumHouseCheck";
mySubcomponents[aux_create+i] = aux_cmp_num;
CheckTotal_8_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 18 ;
aux_cmp_num += 4;
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
CheckTotal_8_run(mySubcomponents[cmp_index_ref],ctx);
}
{
